...
- **Bot Personality System**: 19 unique bot profiles with distinct personalities, backstories, and language specializations. Each bot has a personality trait that influences their conversation style. **Practice mode bot selection**: Users can choose specific bots before starting practice matches via a dialog showing bot details (rating, backstory, description). Bots are language-specific (Chinese, Spanish, Italian) or multilingual. Bot personality is incorporated into AI prompts for more authentic and varied conversations. Competitive mode randomly selects appropriate bots for the language.
